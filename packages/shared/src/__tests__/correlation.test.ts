...
describe('buildBreachKey', () => {
  it('produces a stable key for the same hour + symbol + type', () => {
    ...
    const b = buildBreachKey({
      type: 'SINGLE_STOCK_EXPOSURE',
      symbol: 'AAPL',
      asOf: '2026-05-18T10:59:59.999Z',
    });
    expect(a).toBe(b);
    expect(a).toBe('SINGLE_STOCK_EXPOSURE#AAPL#2026-05-18T10');
  });

  it('changes when the hour rolls over', () => {
    const a = buildBreachKey({ type: 'X', symbol: 'AAPL', asOf: '2026-05-18T10:30:00.000Z' });
    const b = buildBreachKey({ type: 'X', symbol: 'AAPL', asOf: '2026-05-18T11:00:00.000Z' });
    expect(a).not.toBe(b);
  });

  it('uses PORTFOLIO when symbol is omitted', () => {
    const k = buildBreachKey({ type: 'DAILY_PORTFOLIO_DROP', asOf: '2026-05-18T10:00:00.000Z' });
    expect(k).toBe('DAILY_PORTFOLIO_DROP#PORTFOLIO#2026-05-18T10');
  });
});
